...
%% Filter specifications EOG
low_cutoff_eog=0.1; % band pass filter lower cutoff frequency (Hz)
upper_cutoff_eog=30; % band pass filter upper cutof f frequency (Hz)

filter_order = 3;                   
filter_cutoff = [2*low_cutoff_eog/fa 2*upper_cutoff_eog/fa];

[b_eog, a_eog] = butter(filter_order,filter_cutoff); 
...
%         ch=ch+10*cos(2*pi*10*stamps);
        ...
        if(~isnan(circBuff_1))
            ...
            circBuff_2=filtfilt(b_eog, a_eog, circBuff_2);
        end
        ...
        subplot(3,1,2);
        plot(t_circBuff, circBuff_2, 'b', 'Linewidth', 3);
        axis([-inf inf -350 350]);
        xlabel('Time (s)');
