...
for i=1:length(indiceA)
    PA(:,:,i) = 0.5 * (FT(:, 1:m/2, indiceA(i)).^2 + FT(:, m/2 +1:end, indiceA(i)).^2);
end

[Hf,faixaFreq] = gera_Ho_diag(faixa,coef,m,banda,incremento,flag, PA);
...
% for i=1:length(indiceA)
%     PA(:,:,i) = 0.5 * (FE(:, 1:m/2, indiceA(i)).^2 + FE(:, m/2 +1:end, indiceA(i)).^2);
% end
% 
% [Hf,faixaFreq] = gera_Ho_diag(faixa,coef,m,banda,incremento,flag, PA);
